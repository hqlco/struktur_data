// sudah tau bertetangga gimana maksutnya
#include <stdlib.h>
#include <stdbool.h>
#include <stdio.h>
#include <string.h>
...
int find(BSTNode *root, int t, int l)
{
    if (!root)
        return 0;
    if (root->key == t)
...
int getLevel(BST *bst, int target)
{
    if (!bst)
        return 0;
    int l = 1;
    return find(bst->_root, target, l);
}
void delek(BSTNode *root, int X)
{
    if (root == NULL)
    {
        return;
    ...
    {
        if (root->key + root->left->key + root->right->key == X)
        {
            akan = 1;
            return;
        }
    ...
    {
        if (root->key + root->left->key + root->left->left->key == X)
        {
            akan = 1;
            return;
        }
    ...
    {
        if (root->key + root->right->key + root->right->right->key == X)
        {
            akan = 1;
            return;
        }
    ...
    {
        if (root->key + root->left->key + root->left->right->key == X)
        {
            akan = 1;
            return;
        }
    ...
    {
        if (root->key + root->right->key + root->right->left->key == X)
        {
            akan = 1;
            return;
        }
...
void bst_preorder1(BST *bst, int data)
{
    delek(bst->_root, data);
    if (akan == 1)
    {
        printf("%d | | Pintu terbuka! Lanjut eksplorasi.\n", data);
        return;
...
int main()
{
    ...
    */
    ...
    for (int i = 0; i < ulang; i++)
    {
        int a, b;
        scanf("%d %d", &a, &b);
        int X = getLevel(&set, a);
        int Y = getLevel(&set, b);
        if (X == Y)
        {
